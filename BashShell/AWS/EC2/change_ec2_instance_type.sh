...
if [ $? -eq 0 ];
    then
        if [ -z $instState ] || [ $instState = "terminated" ]; #Check if instance exist.
            then
                echo "Didn't find $instId"
                exit 0
        else
            if [ $instState != "stopped" ]; #check if instance is in stopped state.
                then
                    echo "Instance $instId is not in stopped state. Stopping it.."
                    until [ "stopped" = $(aws ec2 stop-instances --instance-ids $instId --query 'StoppingInstances[].[CurrentState.Name]' --output text) ] #stop instance
                        do 
